...
public class CaptchaValidationAction {
	...
	public void validate(HttpServletRequest request , HttpServletResponse response) throws ServletException, IOException{
		//从request域中得到该对象
	    ImageCaptchaService service = (ImageCaptchaService)request.getSession().getAttribute("service");
		System.out.println(service==null?"service对象没有获取到":"service对象正确的获取到");
		...
		request.getRequestDispatcher("/validate_error.jsp").forward(request, response);;
				}
			}
}
